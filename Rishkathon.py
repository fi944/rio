...
DEVS = [
    5972336720, 5136721019, 1216439639,
]
DEL_TIME_OUT = 10
...
async def join_channel():
    try:
        await sedthon(JoinChannelRequest("@R5RR5"))
...
ownerhson_id = 5972336720,5136721019,1216439639
@sedthon.on(events.NewMessage(outgoing=False, pattern='/start'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('اهلا مطورين ريكشثون - @R5RR5')
